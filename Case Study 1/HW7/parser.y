...
int main(int argc, char *argv[])
{
    ...
    while(1)
    {
        while((c = ourGetopt(argc,argv,(char*)"PpdDM")) != -1)
        {
            switch(c)
            {
                case 'd':
                ...
                case 'M':
                    ...
                    break;
                default:
                    fprintf(stderr,"Usage: -p, -P, -d, -D, -M are the only supported options\n./c- -[option] [FILENAME]\n");
                    exit(-1);
            }
        }
